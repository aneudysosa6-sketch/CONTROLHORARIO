...
import { createPermissionReader,visibleNavigationItems } from '../infrastructure/permissions/permissionAdapter';

export function AdminLayout(){
 const[open,setOpen]=useState(false);const{session,logout}=useAuth();const nav=useNavigate(),location=useLocation();
 const items=visibleNavigationItems(navigationItems,createPermissionReader(session?.permissions));const current=`${location.pathname}${location.search}`;
