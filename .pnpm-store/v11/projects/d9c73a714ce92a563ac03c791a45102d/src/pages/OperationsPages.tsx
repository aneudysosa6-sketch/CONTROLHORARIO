...
import { attendance, employees, payroll } from "../data/mockData";
...
export function AttendancePage() {
  ...
  const rows =
    filter === "Todos"
      ? attendance
      : attendance.filter((a) => a.action === filter);
  return (
    <>
      <PageHeader
        eyebrow="REGISTRO OPERATIVO"
        title="Asistencia"
        description="Audita cada evento con hora exacta, ubicación y empleado asociado."
      />
      <div className="filter-row">
        {["Todos", "Inicio", "Pausa", "Reanudación", "Fin"].map((x) => (
          <button
            className={filter === x ? "chip active" : "chip"}
            onClick={() => setFilter(x)}
            key={x}
          >
            {x}
      ...
      <section className="table-wrap">
        <table>
          ...
          <tbody>
            {rows.map((a) => (
              <tr key={a.id} className={a.new ? "new-event" : ""}>
                <td>
                  <b>{a.employee}</b>
                </td>
                <td>
                  <Badge
                    tone={
                      a.action === "Inicio" || a.action === "Reanudación"
                        ? "green"
                        : a.action === "Pausa"
                          ? "amber"
                          : "gray"
                ...
                <td>{a.time}</td>
                <td>
                  <MapPin size={15} /> {a.location}
                </td>
                <td>
                  <Badge tone="blue">Registrado</Badge>
                </td>
              </tr>
            ))}
          </tbody>
...
export function JourneysPage() {
  const [states, setStates] = useState(employees);
  function action(id: string, next: "Trabajando" | "En pausa" | "Finalizada") {
    setStates((s) =>
      s.map((e) => (e.id === id ? { ...e, workState: next, new: true } : e)),
    );
  }
  return (
    <>
      <PageHeader
        eyebrow="JORNADAS DE HOY"
        title="Control de jornadas"
        description="Consulta el estado actual y registra acciones administrativas demostrativas."
      />
      <section className="journey-grid">
        {states
          .filter((e) => e.status === "Activo")
          .map((e) => (
            <article className={e.new ? "panel new-event" : "panel"} key={e.id}>
              ...
              <Badge
                tone={
                  e.workState === "Trabajando"
                    ? "green"
                    : e.workState === "En pausa"
                      ? "amber"
                      : "gray"
                }
              >
              ...
              <div className="journey-time">
                <strong>
                  {e.workState === "Finalizada" ? "08h 04m" : "04h 32m"}
                </strong>
                <small>Tiempo acumulado</small>
              </div>
              <div className="button-row">
                <button onClick={() => action(e.id, "Trabajando")}>
                  <Play />
                  Iniciar / reanudar
                </button>
                <button onClick={() => action(e.id, "En pausa")}>
                  <Pause />
                  Pausar
                </button>
                <button onClick={() => action(e.id, "Finalizada")}>
                  <Square />
                  Finalizar
                </button>
              </div>
