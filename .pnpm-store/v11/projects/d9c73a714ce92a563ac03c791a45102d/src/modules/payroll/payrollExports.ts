...
export async function downloadFinalExcel(snapshot:PayrollSnapshot,name:string){
 ...
 const details=snapshot.detalles.map(d=>({Empleado:d.nombre_empleado,Código:d.codigo_empleado,Salario:d.sueldo_base,'Horas normales':d.horas_normales,'Horas extras':d.horas_extras,'Horas nocturnas':d.horas_nocturnas,'Pago extras':d.total_horas_extras,'Pago nocturno':d.total_nocturnas,Incentivos:d.incentivos,Descuentos:d.total_descuentos,Impuestos:d.total_impuestos,Bruto:d.bruto,Neto:d.neto}));
 const discounts=snapshot.detalles.map(d=>({Empleado:d.nombre_empleado,Código:d.codigo_empleado,'DESCU-PRES':d.descuento_prestamo,'DESCU-CRED':d.descuento_credito,'ROTUR/FALT':d.rotura_falta,Otros:d.otros_descuentos,Total:d.total_descuentos}));
