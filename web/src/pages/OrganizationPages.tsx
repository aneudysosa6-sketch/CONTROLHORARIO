...
import { administrationService, type AdministrationOverview, type Branch, type Department, type OrganizationData } from '../modules/administration/administrationService';

const emptyOrganization: OrganizationData = { branches: [], departments: [], positions: [], profiles: [], employees: [], roles: [], permissions: [], departmentAssignments: [], rolePermissions: [] };
const reasonMessage = 'Actualización desde Organización ERP';
...
export function CompaniesPage() { const [company, setCompany] = useState<AdministrationOverview['company'] | null>(null); const [editing, setEditing] = useState(false); const [error, setError] = useState(''); useEffect(() => { administrationService.overview().then((value) => setCompany(value.company)).catch((reason) => setError(reason instanceof Error ? reason.message : 'No fue posible cargar la empresa.')); }, []); const save = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!company) return; try { setCompany(await administrationService.updateCompany({ name: company.name, legal_name: company.legal_name, slug: company.slug, tax_id: company.tax_id, phone: company.phone, email: company.email, address: company.address, logo_url: company.logo_url, status: company.status }, reasonMessage)); setEditing(false); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No fue posible guardar la empresa.'); } }; if (!company) return <Empty text={error || 'Cargando empresa…'} />; return <><PageHeader eyebrow="ORGANIZACIÓN" title="Empresas" description="Empresa visible dentro de la sesión autorizada." action={<Exports />} />{error && <div className="error">{error}</div>}{editing ? <form className="panel admin-form" onSubmit={save}><h2>Editar empresa</h2><label>Código<input value={company.slug} onChange={(event) => setCompany({ ...company, slug: event.target.value })} required /></label><label>Nombre<input value={company.name} onChange={(event) => setCompany({ ...company, name: event.target.value })} required /></label><label>RNC<input value={company.tax_id ?? ''} onChange={(event) => setCompany({ ...company, tax_id: event.target.value })} /></label><label>Teléfono<input value={company.phone ?? ''} onChange={(event) => setCompany({ ...company, phone: event.target.value })} /></label><label>Correo<input type="email" value={company.email ?? ''} onChange={(event) => setCompany({ ...company, email: event.target.value })} /></label><label>Dirección<input value={company.address ?? ''} onChange={(event) => setCompany({ ...company, address: event.target.value })} /></label><label>Logo<input value={company.logo_url ?? ''} onChange={(event) => setCompany({ ...company, logo_url: event.target.value })} /></label><label>Estado<select value={company.status} onChange={(event) => setCompany({ ...company, status: event.target.value })}><option value="active">Activo</option><option value="inactive">Inactivo</option></select></label><button className="primary">Guardar</button></form> : <section className="table-wrap"><table><thead><tr><th>Código</th><th>Nombre</th><th>RNC</th><th>Teléfono</th><th>Correo</th><th>Estado</th><th>Acciones</th></tr></thead><tbody><tr><td>{company.slug}</td><td>{company.name}</td><td>{company.tax_id ?? '—'}</td><td>{company.phone ?? '—'}</td><td>{company.email ?? '—'}</td><td><Badge tone={company.status === 'active' ? 'green' : 'gray'}>{company.status}</Badge></td><td><button className="secondary" onClick={() => setEditing(true)}>Editar</button></td></tr></tbody></table></section>}</>; }
