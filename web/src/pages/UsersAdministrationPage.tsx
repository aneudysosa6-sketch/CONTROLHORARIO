...
function scopeValidationMessage(code: string) {
  ...
  if (code === 'SUPERVISOR_BRANCH_REQUIRED') return 'Selecciona al menos una sucursal supervisada.';
  if (code === 'SUPERVISOR_BRANCH_WITHOUT_DEPARTMENTS') return 'Selecciona al menos un departamento de cada sucursal supervisada.';
...
function SupervisorScopeFields({
  ...
  onSelectAll: (branchId: string) => void;
  onClear: (branchId: string) => void;
  onRetry: () => void;
}) {
  return <fieldset className="access-scope" disabled={disabled && !loadError}>
    <legend>Alcance del supervisor</legend>
    <p className="access-scope-help">El acceso queda limitado exclusivamente a los departamentos seleccionados en una o varias sucursales.</p>
    ...
    {needsReconciliation && <div className="error access-scope-state" role="alert">Las asignaciones existentes incluyen datos inactivos o inconsistentes. Revisa el alcance antes de guardar.</div>}
    {!loading && !loadError && <>
      <fieldset className="access-departments">
        <legend>Sucursales supervisadas</legend>
        <div className="access-department-list">
          {(data?.branches ?? []).map((branch) => <label className="access-department-option" key={branch.id}>
            <input type="checkbox" checked={branchIds.includes(branch.id)} onChange={(event) => onToggleBranch(branch.id, event.target.checked)} />
            <span>{branch.name}</span>
          </label>)}
        </div>
      </fieldset>
      {!data?.branches.length && <div className="access-empty-note">No hay sucursales activas disponibles.</div>}
      {!branchIds.length && <div className="access-scope-state">Selecciona una o varias sucursales para configurar sus departamentos.</div>}
      {branchIds.map((branchId) => {
        ...
        const selectedCount = departmentIds.filter((id) => departments.some((item) => item.id === id)).length;
        return <fieldset className="access-departments" key={branchId}>
          <legend>{branch?.name ?? 'Sucursal'}: departamentos</legend>
          <div className="access-scope-toolbar">
            <span aria-live="polite">{selectedCount} {selectedCount === 1 ? 'departamento seleccionado' : 'departamentos seleccionados'}</span>
            <div className="button-row">
              <button type="button" className="secondary" disabled={!departments.length} onClick={() => onSelectAll(branchId)}>Seleccionar todos</button>
              <button type="button" className="secondary" disabled={!selectedCount} onClick={() => onClear(branchId)}>Limpiar</button>
            </div>
          </div>
          {!departments.length && <div className="access-empty-note">Esta sucursal no tiene departamentos activos.</div>}
          {!!departments.length && <div className="access-department-list">
            {departments.map((department) => <label className="access-department-option" key={department.id}>
              <input type="checkbox" checked={departmentIds.includes(department.id)} onChange={(event) => onToggleDepartment(department.id, event.target.checked)} />
...
export function UsersAdministrationPage({
  ...
  const [supervisorBranchIds, setSupervisorBranchIds] = useState<string[]>([]);
  ...
  async function loadSupervisorScope(profileId?: string) {
    ...
    try {
      ...
      const selectedBranches = [...new Set(data.scope.branch_ids)];
      const allowed = new Set(
        data.departments
          .filter((department) => department.is_active && department.branch_id != null && selectedBranches.includes(department.branch_id))
          .map((department) => department.id),
      );
      setSupervisorScope(data);
      setSupervisorBranchIds(selectedBranches);
  ...
  useEffect(() => {
    ...
    setSupervisorBranchIds([]);
    setSupervisorDepartmentIds([]);
  }, [mode, creating, editIsSupervisor, createIsSupervisor]);

  function toggleScopeBranch(branchId: string, selected: boolean) {
    const next = toggleSupervisorBranch(
      { branchIds: supervisorBranchIds, departmentIds: supervisorDepartmentIds },
      branchId,
      selected,
      supervisorScope?.departments ?? [],
    );
    setSupervisorBranchIds([...next.branchIds]);
  ...
  function selectAllScopeDepartments(branchId: string) {
    setSupervisorDepartmentIds((current) => [...new Set([
      ...current,
      ...selectAllActiveDepartmentIds(supervisorScope?.departments ?? [], [branchId]),
    ])]);
    setScopeNeedsReconciliation(false);
  }

  function clearScopeDepartments(branchId: string) {
    const branchDepartmentIds = new Set(
      activeDepartmentsForBranch(supervisorScope?.departments ?? [], branchId).map((department) => department.id),
    );
    setSupervisorDepartmentIds((current) => current.filter((id) => !branchDepartmentIds.has(id)));
    setScopeNeedsReconciliation(false);
  }
      ...
      <form className="form-panel access-form" onSubmit={saveAccess}>
        <div className="form-grid access-form-grid access-edit-grid">
          ...
          {editIsSupervisor && <SupervisorScopeFields
            ...
            onToggleBranch={toggleScopeBranch}
    ...
    {creating && <div className="access-dialog-backdrop" role="presentation">
      <form ref={createDialogRef} className="panel access-dialog access-dialog-wide" role="dialog" aria-modal="true" aria-labelledby="create-user-title" onKeyDown={(event) => trapDialogFocus(event, createDialogRef, closeCreateDialog)} onSubmit={createAccess}>
        ...
        <div className="form-grid access-form-grid">
          ...
          <label>Usuario
            <input autoFocus type="email" autoComplete="username" value={newUsername} onChange={(event) => setNewUsername(event.target.value)} placeholder="usuario@example.invalid" required />
          </label>
          <label>Contraseña
