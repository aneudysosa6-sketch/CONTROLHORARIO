...
export const administrationService={
 ...
 async saveDepartment(id:string|null,data:Record<string,unknown>,supervisor:string|null,reason:string){const result=await rpc<string>('guardar_departamento_administracion',{p_id:id,p_datos:data,p_supervisor:supervisor,p_motivo:reason});const{publishNotification}=await import('../notifications/notificationIntegration');publishNotification('department_changed','Departamento actualizado',{departmentId:result,supervisorId:supervisor},`department-changed:${result}:${supervisor??'none'}:${reason}`);if(supervisor)publishNotification('supervisor_changed','Supervisor asignado',{departmentId:result,supervisorId:supervisor},`supervisor-changed:${result}:${supervisor}`);return result},
 ...
 async saveRole(id:string|null,name:string,code:string,description:string,active:boolean,reason:string){const normalizedName=name.trim();const normalizedCode=code.trim().toLowerCase().replace(/[^a-z0-9_]+/g,'_').replace(/^_+|_+$/g,'');if(!normalizedName)throw new AdministrationError('guardar_rol_administracion','ROLE_NAME_REQUIRED','El nombre del rol es obligatorio.');if(!normalizedCode)throw new AdministrationError('guardar_rol_administracion','ROLE_CODE_REQUIRED','El código del rol es obligatorio.');return rpc<string>('guardar_rol_administracion',{p_id:id,p_nombre:normalizedName,p_codigo:normalizedCode,p_descripcion:description.trim(),p_activo:active,p_motivo:reason.trim()||'Creación inicial de usuario'})},
