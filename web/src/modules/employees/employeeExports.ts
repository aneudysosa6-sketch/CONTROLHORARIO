import { safeSpreadsheetText } from '../reports/spreadsheetText';
import type { EmployeeRecord } from './employeeService';

const commonColumns = (employee: EmployeeRecord) => ({
  Código: safeSpreadsheetText(employee.code),
  Empleado: safeSpreadsheetText(employee.name),
  Cédula: safeSpreadsheetText(employee.cedula),
  Empresa: safeSpreadsheetText(employee.companyName),
  Sucursal: safeSpreadsheetText(employee.branchName),
  Departamento: safeSpreadsheetText(employee.departmentName),
  Cargo: safeSpreadsheetText(employee.positionName),
  Estado: safeSpreadsheetText(employee.status),
});

const exportRows = (employees: EmployeeRecord[], includeTermination: boolean) => employees.map((employee) => ({
  ...commonColumns(employee),
  ...(includeTermination ? {
    'Fecha de desvinculación': safeSpreadsheetText(employee.terminationDate),
    'Motivo de desvinculación': safeSpreadsheetText(employee.terminationReason),
    Observación: safeSpreadsheetText(employee.terminationObservation),
    Responsable: safeSpreadsheetText(employee.terminationActorName),
  } : {}),
}));
