...
export const payrollService={
 ...
 async dashboardTotal(workDate:string){
  try{
   ...
   if(import.meta.env.DEV)console.info('PAYROLL_DASHBOARD',{
   ...
   if(import.meta.env.DEV)console.error('PAYROLL_DASHBOARD',{empleadosIncluidos:0,jornadasUtilizadas:0,horas:0,salario:[],bloqueos:[],totalCalculado:null,motivo:error instanceof Error?error.message:'PAYROLL_DASHBOARD_RPC_ERROR'});
   throw error;
  }
 },
 ...
 async calculate(periodId:string){const result=await rpc<{periodo_id:string;nomina_id:string;version:number;resumen:PayrollSummary}>('calcular_nomina_p0',{p_periodo:periodId});const{publishNotification}=await import('../notifications/notificationIntegration');publishNotification('payroll_generated','Nómina generada',{periodId,result},`payroll-generated:${periodId}:${result.version}`);return result},
