...
export type PayrollDetail={id:string;empleado_id:string;codigo_empleado:string;nombre_empleado:string;tipo_pago:string;sueldo_base:number;dias_trabajados:number;horas_normales:number;horas_extras:number;horas_nocturnas:number;valor_hora:number;total_horas_extras:number;total_nocturnas:number;incentivos:number;afp:number;sfs:number;otros_impuestos:number;total_impuestos:number;descuento_prestamo:number;descuento_credito:number;rotura_falta:number;otros_descuentos:number;total_descuentos:number;bruto:number;neto:number;version_calculo:number;formula:string};
...
export type PayrollRule={empleado_id?:string;dias_divisor_quincenal:number;horas_dia:number;porcentaje_extra:number;porcentaje_nocturno:number;afp_modo:'MONTO'|'PORCENTAJE';afp_valor:number;sfs_modo:'MONTO'|'PORCENTAJE';sfs_valor:number;otros_impuestos_modo:'MONTO'|'PORCENTAJE';otros_impuestos_valor:number;incentivo_periodo:number;version:number};
export type PayrollRules={global:PayrollRule;empleados:PayrollRule[]};
export type PayrollSnapshot={periodo:PayrollPeriod;nomina:{id:string;estado:PayrollState;version_calculo:number;formula:string;desactualizada:boolean;errores:unknown[];resumen:PayrollSummary;calculada_en?:string};detalles:PayrollDetail[];ajustes:unknown[];prestamos:PayrollLoan[];creditos:PayrollCredit[];auditoria:PayrollAudit[];reglas:PayrollRules};
...
export const payrollService={
 ...
 async get(periodId:string){const[snapshot,reglas]=await Promise.all([rpc<Omit<PayrollSnapshot,'reglas'>>('obtener_nomina',{p_periodo:periodId}),rpc<PayrollRules>('obtener_reglas_nomina')]);return{...snapshot,reglas}},
 ...
 calculate:(periodId:string)=>rpc<{periodo_id:string;nomina_id:string;version:number;resumen:PayrollSummary}>('calcular_nomina',{p_periodo:periodId}),
 ...
 configureRule:(employeeId:string|null,config:Omit<PayrollRule,'empleado_id'|'version'>,reason:string)=>rpc<void>('configurar_regla_nomina',{p_empleado:employeeId,p_config:config,p_motivo:reason}),
 ...
 changeLoanState:(id:string,state:'APROBADO'|'ENTREGADO'|'CANCELADO',reason:string)=>rpc<void>('cambiar_estado_prestamo_nomina',{p_prestamo:id,p_estado:state,p_motivo:reason}),
