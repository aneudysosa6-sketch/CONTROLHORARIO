...
@Dao interface DeviceEnrollmentDao{
 ...
 @Query("UPDATE device_enrollment SET branchId=:branchId, usageType=:usageType, departmentIds=:departmentIds, voiceEnabled=:voiceEnabled, configurationRevision=:configurationRevision WHERE deviceId=:deviceId")
 suspend fun recordTerminalConfiguration(deviceId:String,branchId:String?,usageType:String,departmentIds:String,voiceEnabled:Boolean,configurationRevision:Long)
 @Query("DELETE FROM device_enrollment") suspend fun clear()
}
