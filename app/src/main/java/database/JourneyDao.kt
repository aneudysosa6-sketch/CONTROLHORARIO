...
interface JourneyDao {
    ...
    @Transaction
    suspend fun recordAction(employeeLocalId:Int,employeeRemoteId:String,employeeName:String,deviceId:String,workDate:String,occurredAt:String,action:JourneyAction,jornadaEnabled:Boolean):LocalJourneyResult {
        require(jornadaEnabled){"ATTENDANCE_DISABLED"}
        val existing=find(employeeLocalId,workDate)
        val current=existing?.toSnapshot()?:JourneySnapshot()
        val transition=JourneyStateEngine.apply(current,action,occurredAt)
        require(transition.accepted){transition.errorCode?:"INVALID_TRANSITION"}
        val updated=(existing?:JourneyEntity(employeeLocalId=employeeLocalId,employeeRemoteId=employeeRemoteId,deviceId=deviceId,workDate=workDate)).fromSnapshot(transition.snapshot).copy(syncStatus="PENDIENTE",updatedAt=System.currentTimeMillis(),jornadaEnabledSnapshot=jornadaEnabled)
        val localId=if(existing==null)insertJourney(updated).toInt() else{updateJourney(updated);updated.localId}
        val key=UUID.randomUUID().toString()
        val payload=JSONObject().put("contract_version",1).put("employee_remote_id",employeeRemoteId).put("action",action.name).put("occurred_at",occurredAt).put("work_date",workDate).put("idempotency_key",key).put("known_version",existing?.syncVersion?:0).toString()
        val outboxId=insertOutbox(JourneyOutboxEntity(journeyLocalId=localId,operation=action.name,idempotencyKey=key,payload=payload))
        insertLegacy(AttendanceEntity(employeeId=employeeLocalId,employeeName=employeeName,date=workDate,time=occurredAt.substringAfter('T').take(8),actionType=action.legacyAction(),biometricVerified=true,deviceName="2Connect USB Fingerprint Scanner",notes="RC2 offline; outbox=$key"))
        return LocalJourneyResult((if(existing==null)updated.copy(localId=localId) else updated),outboxId)
    }
}
