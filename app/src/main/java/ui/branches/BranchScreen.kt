            ...
            items(branches) { branch ->
                ...
                Card(
                    ...
                    Column(
                        ...
                        Spacer(modifier = Modifier.height(8.dp))
                        Text("Estado: " + if (branch.active) "ACTIVA" else "INACTIVA")
                        val activeDepartmentCount = departments.count { it.branchId == branch.id && it.active }
                        if (branch.active && activeDepartmentCount > 0) {
                            Text("Desactiva primero " + activeDepartmentCount + " departamento(s) activo(s).")
                        }
                        Button(
                            modifier = Modifier.fillMaxWidth(),
                            enabled = !branch.active || activeDepartmentCount == 0,
                            onClick = { viewModel.setBranchActive(branch, activeDepartmentCount) }
                        ) {
                            Text(if (branch.active) "Desactivar" else "Reactivar")
                        }
                    }
                }
            }
