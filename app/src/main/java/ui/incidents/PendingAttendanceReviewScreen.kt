...
fun PendingAttendanceReviewScreen(
    ...
    OSINETScreen {
        OSINETHeader(
            ...
            subtitle = "Resolver como NO PAGAR o por evidencia demostrable"
        )
        ...
        if (ordered.isEmpty()) {
            OSINETCard { Text("No hay jornadas incompletas pendientes de resolución.", color = OSINETColors.TextSecondary) }
        }
        ordered.forEach { review ->
            PendingReviewCard(
                review = review,
                canReview = review.status == PendingAttendanceReviewEntity.STATUS_PENDING,
                onNoPay = { viewModel.resolveNoPay(review) },
                onResolve = { manualHours -> viewModel.resolveDemonstrable(review, manualHours) }
            )
            Spacer(Modifier.height(10.dp))
        }
        Spacer(Modifier.height(18.dp))
...
private fun PendingReviewCard(
    ...
    var manualHours by remember(review.id) { mutableStateOf("") }
    OSINETCard {
        ...
        Text("Intervalos demostrables: ${"%.2f".format(review.calculatedHours)} horas", color = OSINETColors.Warning)
        ...
        if (canReview) {
            ...
            if (onlyStart) {
                OSINETTextField(
                    value = manualHours,
                    onValueChange = { manualHours = it.filter { character -> character.isDigit() || character == '.' }.take(4) },
                    label = "Horas manuales (0 a 8)",
                    modifier = Modifier.fillMaxWidth()
                )
                Spacer(Modifier.height(8.dp))
            }
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                OutlinedButton(onClick = onNoPay) { Text("NO PAGAR") }
                OutlinedButton(
                    enabled = !onlyStart || manualHours.toDoubleOrNull()?.let { it in 0.0..8.0 } == true,
                    onClick = { onResolve(if (onlyStart) manualHours.toDoubleOrNull() else null) }
                ) {
                    Text(if (onlyStart) "GUARDAR HORAS" else "USAR INTERVALOS")
                }
            }
        }
    }
