...
fun Rc2EmployeeAttendanceScreen(employee:Employee?,viewModel:JourneyViewModel,onFinish:()->Unit){
 BackHandler(onBack=onFinish);val journey by viewModel.journey.collectAsState();val busy by viewModel.busy.collectAsState();val error by viewModel.error.collectAsState();val authorized by viewModel.isPunchAuthorized.collectAsState();val status=journey?.status?.let{runCatching{JourneyStatus.valueOf(it)}.getOrNull()}?:JourneyStatus.SIN_INICIAR;val allowed=JourneyStateEngine.allowedActions(status)
 OSINETScreen{OSINETHeader("Registrar jornada","Selecciona una acción permitida");Spacer(Modifier.height(18.dp));OSINETCard{Text(employee?.nombre?:"Empleado no encontrado",color=OSINETColors.TextPrimary,fontWeight=FontWeight.SemiBold);Text("Estado actual: ${statusLabel(status)}",color=OSINETColors.GreenSoft);Text("Trabajado: ${journey?.workedMinutes?:0} min · Pausa: ${journey?.breakMinutes?:0} min",color=OSINETColors.TextSecondary)};Spacer(Modifier.height(14.dp));Text(if(status==JourneyStatus.FINALIZADA)"La jornada de hoy ya fue finalizada." else if(authorized)"Rostro validado. Registra una sola acción." else "Autorización vencida. Vuelva al PIN.",color=OSINETColors.TextSecondary,textAlign=TextAlign.Center,modifier=Modifier.fillMaxWidth());Spacer(Modifier.height(18.dp))
  actionButton("Iniciar Jornada",JourneyAction.INICIAR,allowed,busy,authorized,employee,viewModel,onFinish);Spacer(Modifier.height(10.dp));actionButton("Pausar Jornada",JourneyAction.PAUSAR,allowed,busy,authorized,employee,viewModel,onFinish);Spacer(Modifier.height(10.dp));actionButton("Reanudar Jornada",JourneyAction.REANUDAR,allowed,busy,authorized,employee,viewModel,onFinish);Spacer(Modifier.height(10.dp));actionButton("Finalizar Jornada",JourneyAction.FINALIZAR,allowed,busy,authorized,employee,viewModel,onFinish)
...
@Composable private fun actionButton(text:String,action:JourneyAction,allowed:Set<JourneyAction>,busy:Boolean,authorized:Boolean,employee:Employee?,viewModel:JourneyViewModel,onFinish:()->Unit)=OSINETButton(text,{employee?.let{viewModel.record(it,LocalDate.now().toString(),action,onFinish)}},enabled=employee!=null&&!busy&&authorized&&action in allowed)
