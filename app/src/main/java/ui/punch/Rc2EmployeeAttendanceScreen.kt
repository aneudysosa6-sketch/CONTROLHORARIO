...
fun Rc2EmployeeAttendanceScreen(
    ...
    LaunchedEffect(viewModel, employee?.id) {
    ...
    val stateMessage = when {
        ...
        status == JourneyStatus.FINALIZADA -> "JORNADA YA FINALIZADA"
    ...
    OSINETScreen {
        ...
        Spacer(Modifier.height(18.dp))
        OSINETSecondaryButton("Cancelar", onCancel)
    }
}
...
private fun JourneyActionButton(
    ...
    text = text,
    onClick = {
        employee?.let { currentEmployee ->
            viewModel.record(currentEmployee, action) {
                onActionSaved(currentEmployee.nombre, action)
            }
        }
    },
