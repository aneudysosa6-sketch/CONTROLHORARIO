            ...
            items(departments) { department ->
                ...
                Card(
                    ...
                    Column(
                        ...
                        Spacer(modifier = Modifier.height(8.dp))
                        Text("Estado: " + if (department.active) "ACTIVO" else "INACTIVO")
                        val activeEmployeeCount = employees.count {
                            it.departmentId == department.id && it.isActive &&
                                !it.employmentStatus.orEmpty().equals("desvinculado", ignoreCase = true)
                        }
                        if (department.active && activeEmployeeCount > 0) {
                            Text("Mueve primero " + activeEmployeeCount + " empleado(s) activo(s).")
                        }
                        Button(
                            modifier = Modifier.fillMaxWidth(),
                            enabled = !department.active || activeEmployeeCount == 0,
                            onClick = { viewModel.setDepartmentActive(department, activeEmployeeCount) }
                        ) {
                            Text(if (department.active) "Desactivar" else "Reactivar")
                        }
                    }
                }
            }
