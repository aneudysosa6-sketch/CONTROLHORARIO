...
fun EmployeePermissionRequestsScreen(
    ...
    OSINETScreen {
        OSINETHeader(
            ...
            subtitle = "Gestionar permisos y registrar licencias directas"
        )
...
private fun EmployeePermissionRow(
    ...
    val normalDailyAmount = (employee?.sueldo ?: 0.0) / 30.0
    ...
    OSINETCard {
        ...
        val isDirectLicense = request.requestType == EmployeePermissionRequestEntity.TYPE_MEDICAL_LICENSE
        if (isDirectLicense && request.status in setOf(
        ...
        if (isDirectLicense && request.status in setOf(
            ...
            Spacer(Modifier.height(8.dp))
            Text("Pago diario (salario / 30): ${money(dailyLicenseAmount)}", color = OSINETColors.GreenSoft)
            Spacer(Modifier.height(8.dp))
            OSINETButton(
                if (request.status == EmployeePermissionRequestEntity.STATUS_ACTIVE) "GUARDAR CAMBIOS" else "REGISTRAR LICENCIA",
                enabled = licenseStart.isNotBlank() && licenseEnd.isNotBlank() && percentValue in 0.0..100.0,
                onClick = {
                    viewModel.saveDirectLicense(
                        request = request,
                        actor = reviewerName,
                        now = nowDateTime(),
                        ...
                        monthlySalary = employee?.sueldo ?: 0.0
                    )
                }
            )
            if (request.status == EmployeePermissionRequestEntity.STATUS_ACTIVE) {
                Spacer(Modifier.height(8.dp))
                OSINETSecondaryButton("CANCELAR LICENCIA", onClick = { showRejectReason = true })
                if (showRejectReason) {
                    Spacer(Modifier.height(8.dp))
                    OSINETTextField(rejectReason, { rejectReason = it }, "Motivo de cancelación", Modifier.fillMaxWidth())
                    Spacer(Modifier.height(8.dp))
                    OSINETButton(
                        "CONFIRMAR CANCELACIÓN",
                        enabled = rejectReason.isNotBlank(),
                        onClick = {
                            viewModel.cancelDirectLicense(
        ...
        } else if (!isDirectLicense && request.status == EmployeePermissionRequestEntity.STATUS_PENDING) {
            Spacer(Modifier.height(8.dp))
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OSINETButton(
                    "APROBAR",
                    onClick = { viewModel.approveRequest(request, reviewerName, nowDateTime()) },
            ...
            if (showRejectReason) {
                ...
                OSINETButton(
                    ...
                    enabled = rejectReason.isNotBlank(),
                    onClick = {
                        viewModel.rejectRequest(request.id, reviewerName, rejectReason.trim(), nowDateTime())
                    }
                )
            }
        }
