...
fun FaceRegistrationScreen(
    ...
    OSINETScreen {
        ...
        Text("La captura exige un solo rostro, calidad válida y prueba de vida por parpadeo.", color = colors.onSurfaceVariant)
        Spacer(Modifier.height(12.dp))
        Text("Empleado: ${state.employee?.nombre.orEmpty()}", color = colors.onSurface)
        Text("Código: ${state.employee?.employeeCode ?: initialEmployeeCode}", color = colors.onSurfaceVariant)
        Text("Registrar rostro para este empleado", color = colors.primary)
        ...
        AnimatedContent(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                ...
                Text("Muestra ${(state.samples + 1).coerceAtMost(FaceRegistrationPose.entries.size)} de ${FaceRegistrationPose.entries.size}", style = MaterialTheme.typography.titleMedium, color = colors.primary)
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Button(
                ...
                Text(
                    ...
                    else if (state.cameraError) "Reintentar pose"
        ...
        if (state.capturing && cameraGranted) {
            FaceRegistrationCamera(
                ...
                onLivenessVerified = viewModel::confirmLiveness,
                onError = viewModel::cameraFailure,
                pose = state.currentPose,
                completed = state.samples
            )
            if (state.livenessVerified) Text("Prueba de vida completada", color = colors.primary)
...
private fun FaceRegistrationCamera(
    ...
    val liveness = remember { FaceRegistrationLivenessPolicy() }
                                    ...
                                    when {
                                        ...
                                        faces.size > 1 -> {
                                            ...
                                            when {
                                                ...
                                                !liveness.isVerified -> {
                                                    when (val live = liveness.observe(face.leftEyeOpenProbability, face.rightEyeOpenProbability)) {
                                                        is FaceRegistrationLivenessPolicy.Observation.Waiting ->
                                                            postIfActive(active, mainExecutor) { onGuidance(live.guidance) }
                                                        FaceRegistrationLivenessPolicy.Observation.Verified ->
                                                            postIfActive(active, mainExecutor) { onLivenessVerified() }
                                                    }
                                                }
