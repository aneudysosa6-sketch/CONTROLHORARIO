...
class DeviceEnrollmentClient(private val endpoint:String){
 fun enroll(code:String,identity:DeviceIdentityManager):EnrollmentResult{
  val body=JSONObject().put("action","exchange").put("code",code.trim().uppercase())
   ...
   .put("name","CONTROL HORARIO Android").put("model",Build.MODEL).put("android_version",Build.VERSION.RELEASE).put("app_version","1.0")
  val json=post(body)
 ...
 private fun post(body:JSONObject,headers:Map<String,String> = emptyMap()):JSONObject{
  ...
  try{
   ...
   Log.d(TAG,"Respuesta recibida: status="+statusCode+" bytes="+response.length)
   val json=try{
    JSONObject(response)
   ...
   Log.d(TAG,"Respuesta de Edge Function procesada: hasError="+edgeMessage.isNotBlank())
