...
import com.example.controlhorario.messages.EmployeeMessagePolicy
...
data class EmployeeMessagePayload(
    val id: String,
...
data class AttendanceSyncResult(
    ...
    val message: EmployeeMessagePayload? = null
)
...
class AttendanceSyncClient(private val endpoint: String) : AttendanceSyncGateway {
    override fun upload(deviceId: String, credential: String, operations: List<JourneyOutboxEntity>): List<AttendanceSyncResult> {
        ...
        try {
            ...
            apiLog("endpoint=" + endpoint + " httpStatus=" + status + " responseBytes=" + response.length)
            ...
            return (0 until json.length()).map { json.getJSONObject(it) }.map { row ->
                ...
                AttendanceSyncResult(
                    row.optString("idempotency_key"),
                    row.optString("result"),
                    row.optString("error_code").takeIf(String::isNotBlank),
                    version,
                    remote?.let { parseRemote(it, version) },
                    remote?.toString().orEmpty(),
                    row.optJSONObject("message")?.let(::parseEmployeeMessage)
                )
            }
        } finally {
    ...
    private fun parseEmployeeMessage(json: JSONObject): EmployeeMessagePayload {
        val id = json.getString("id")
        val employeeId = json.getString("employee_id")
        val type = json.getString("type")
        val duration = json.optInt("audio_duration_seconds", 0)
        require(runCatching { UUID.fromString(id) }.isSuccess) { "INVALID_EMPLOYEE_MESSAGE" }
        require(runCatching { UUID.fromString(employeeId) }.isSuccess) { "INVALID_EMPLOYEE_MESSAGE" }
        require(type in setOf("TEXTO", "VOZ_SISTEMA", "VOZ_GRABADA")) { "INVALID_EMPLOYEE_MESSAGE" }
        val text = json.optString("text").takeIf(String::isNotBlank)
        val audioUrl = json.optString("audio_url").takeIf(String::isNotBlank)
        require(
            (type != "VOZ_GRABADA" && text != null) ||
                (type == "VOZ_GRABADA" && audioUrl != null && duration in 1..30)
        ) { "INVALID_EMPLOYEE_MESSAGE" }
        return EmployeeMessagePayload(id, employeeId, type, text, audioUrl, duration)
    }
