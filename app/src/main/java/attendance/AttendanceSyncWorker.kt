...
class AttendanceSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
            ...
            try {
                ...
                when (result.result) {
                    "accepted", "duplicate" -> {
                        ...
                        result.message?.let { EmployeeMessageInbox.publish(applicationContext, it) }
                    ...
                    "conflict" -> {
                        ...
                        reconcileRejectedJourney(dao, client, deviceId, credential, item)
                ...
                if (error is AttendanceSyncHttpException && error.status in setOf(401, 403)) {
                    TerminalAuthorizationManager.block(
                        if (error.status == 403) "DEVICE_REVOKED" else "DEVICE_CREDENTIAL_INVALID",
                    )
                }
    ...
    private suspend fun reconcileRejectedJourney(
        dao: JourneyDao,
        client: AttendanceSyncClient,
        deviceId: String,
        credential: String,
        item: JourneyOutboxEntity,
    ) {
        val hasPending = dao.pendingCountForJourney(item.journeyLocalId) > 0
        if (!JourneyTerminalRejectionRecoveryPolicy.shouldFetchCanonical(true, hasPending)) return
        try {
            val local = dao.findByLocalId(item.journeyLocalId) ?: return
            val current = withContext(Dispatchers.IO) {
                client.fetchCurrentState(deviceId, credential, local.employeeRemoteId, Instant.now().toString())
            }
            val remote = current.remote ?: return
            dao.hydrateRemoteState(
                local.employeeLocalId,
                local.employeeRemoteId,
                deviceId,
                current.workDate,
                remote.id,
                remote.status,
                remote.startedAt,
                remote.pauseStartedAt,
                remote.pauseEndedAt,
                remote.finishedAt,
                remote.workedMinutes,
                remote.breakMinutes,
                remote.version,
                System.currentTimeMillis(),
            )
            syncLog("employeeId=${local.employeeRemoteId} localRecordId=${item.journeyLocalId} rejectedStateReconciled=${remote.status} version=${remote.version}")
        } catch (error: Exception) {
            syncLog("employeeId=${employeeId(item)} localRecordId=${item.journeyLocalId} rejectedStateReconciliation=DEFERRED errorType=${error.javaClass.simpleName}")
        }
    }
