...
class AttendanceSyncWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    override suspend fun doWork(): Result {
        ...
        return try {
            val results = AttendanceSyncClient(applicationContext.getString(R.string.attendance_sync_url))
                .upload(deviceId, credential, pending)
            results.forEach { result ->
                val item = pending.firstOrNull { it.idempotencyKey == result.idempotencyKey } ?: return@forEach
                val now = System.currentTimeMillis()
                when (result.result) {
                    "accepted", "duplicate" -> {
                        result.remote?.let {
                            dao.applyRemote(item.journeyLocalId, it.id, it.status, it.startedAt, it.pauseStartedAt, it.pauseEndedAt, it.finishedAt, it.workedMinutes, it.breakMinutes, result.version, now)
                        }
                        dao.markSent(item.id, now)
                    ...
                    "conflict" -> {
                        dao.insertConflict(JourneyConflictEntity(journeyLocalId = item.journeyLocalId, idempotencyKey = item.idempotencyKey, localSnapshot = item.payload, remoteSnapshot = result.rawRemote, reason = result.errorCode ?: "VERSION_CONFLICT"))
                        dao.markFailed(item.id, "CONFLICTO", Long.MAX_VALUE, result.errorCode ?: "VERSION_CONFLICT")
                        ...
                        dao.markFailed(item.id, "RECHAZADA", Long.MAX_VALUE, result.errorCode ?: "REJECTED")
