...
internal object JourneyTerminalRejectionRecoveryPolicy {
    fun shouldFetchCanonical(terminalRejection:Boolean,hasPendingOutbox:Boolean):Boolean =
        terminalRejection && !hasPendingOutbox
}
...
class JourneyCurrentStateSynchronizer(
    ...
    private suspend fun refreshLocked(employeeLocalId:Int,requestedAt:String):JourneyCurrentStateRefreshResult {
        ...
        if(flushConflict && !JourneyTerminalRejectionRecoveryPolicy.shouldFetchCanonical(flushConflict,pendingAfter)){
