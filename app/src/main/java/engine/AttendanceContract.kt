...
object AttendanceContract {
    ...
    const val ERROR_INVALID_TIMESTAMP = "INVALID_TIMESTAMP"
    const val ERROR_NON_MONOTONIC_TIMESTAMP = "NON_MONOTONIC_TIMESTAMP"
}
...
object JourneyStateEngine {
    ...
    fun apply(current: JourneySnapshot, action: JourneyAction, occurredAt: String): JourneyTransition {
        ...
        val eventAt = runCatching { Instant.parse(occurredAt) }
            .getOrElse { return JourneyTransition(false, current, AttendanceContract.ERROR_INVALID_TIMESTAMP) }
        val lastAt = listOf(
            current.startedAt,
            current.pauseStartedAt,
            current.pauseEndedAt,
            current.finishedAt
        ).mapNotNull { value -> value?.let { runCatching { Instant.parse(it) }.getOrNull() } }
            .maxOrNull()
        if (lastAt != null && eventAt.isBefore(lastAt)) {
            return JourneyTransition(false, current, AttendanceContract.ERROR_NON_MONOTONIC_TIMESTAMP)
        }
