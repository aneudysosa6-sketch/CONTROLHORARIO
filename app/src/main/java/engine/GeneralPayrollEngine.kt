...
object GeneralPayrollEngine {
    ...
    fun calculate(
        ...
        val rows = employees
            ...
            .map { employee ->
                ...
                val loanDue = calculateDueDiscount(
                ...
                val creditDue = calculateDueDiscount(
                ...
                val totalGross = PayrollMoneyPolicy.money(
                    employee.sueldo + result.overtimePayment + result.holidayPayment +
                        medicalLicensePayment + incentive
                )
                val debtAllocation = PayrollMoneyPolicy.fundDebtDeductions(
                    gross = totalGross,
                    mandatoryDeductions = taxes + employeeSettings.otherDiscountAmount,
                    loanDue = loanDue,
                    creditDue = creditDue
                )
                val loanDiscount = debtAllocation.loan
                val creditDiscount = debtAllocation.credit
                val totalPay = PayrollMoneyPolicy.money(
                    (totalGross - loanDiscount - creditDiscount - taxes -
                        employeeSettings.otherDiscountAmount).coerceAtLeast(0.0)
                )
    ...
    fun prepareAutoHourPrices(
        ...
        val normalHour = if (employee.sueldo > 0.0) {
            PayrollMoneyPolicy.hourlyRateFromMonthlySalary(employee.sueldo)
        } else {
            base.normalHourPrice
        }
    ...
    private fun calculateDueDiscount(
        total: Double,
        pending: Double,
        configuredDiscount: Double
    ): Double = PayrollMoneyPolicy.dueDiscount(total, pending, configuredDiscount)
