...
object GeneralPayrollExportEngine {
    ...
    fun generateDiscountTemplateCsv(context: Context, employees: List<Employee>): ExportFileResult {
        return try {
            ...
            saveTextToDownloads(
                ...
                mimeType = "text/csv",
                relativeFolder = "CONTROL HORARIO/Nominas",
                content = content
            )
        } catch (e: Exception) {
    ...
    fun generateGeneralPayrollPdf(
        ...
        return try {
            ...
            canvas.drawText(company?.companyName?.ifBlank { "CONTROL HORARIO" } ?: "CONTROL HORARIO", 24f, y, boldPaint)
    ...
    private fun savePdfToDownloads(context: Context, document: PdfDocument, fileName: String): ExportFileResult {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val values = ContentValues().apply {
                ...
                put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/CONTROL HORARIO/Nominas")
                put(MediaStore.Downloads.IS_PENDING, 1)
            }
            val uri = context.contentResolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values)
            ...
            val folder = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "CONTROL HORARIO/Nominas")
            if (!folder.exists()) folder.mkdirs()
            FileOutputStream(File(folder, fileName)).use { document.writeTo(it) }
        }
        document.close()
        return ExportFileResult(true, fileName, "Descargas/CONTROL HORARIO/Nominas/$fileName")
    }

    private fun money(value: Double): String = NumberFormat.getCurrencyInstance(Locale("es", "DO")).format(value)
    private fun csv(value: String): String = SpreadsheetExportPolicy.csvCell(value)
}
