...
class FaceIdentificationEngineTest {
    ...
    @Test
    fun thresholdPassWithNullMarginConfirmsBestCandidate() = runBlocking {
        val engine = engine(
            records = listOf(
                record(employeeId = 1, encryptedEmbedding = "best"),
                record(employeeId = 2, encryptedEmbedding = "close-second")
            ),
            embeddings = mapOf("best" to unit(0.80f), "close-second" to unit(0.79f)),
            margin = null
        )
    ...
    @Test
    fun thresholdFailWithNullMarginReturnsNoMatch() = runBlocking {
        val engine = engine(
            records = listOf(record(employeeId = 1, encryptedEmbedding = "weak-null")),
            embeddings = mapOf("weak-null" to unit(0.70f)),
            margin = null
        )
    ...
    @Test
    fun configuredMarginWithSufficientDifferenceConfirmsMatch() = runBlocking {
        val engine = engine(
            records = listOf(
                record(employeeId = 1, encryptedEmbedding = "best"),
                record(employeeId = 2, encryptedEmbedding = "distant-second")
            ),
            embeddings = mapOf("best" to unit(0.90f), "distant-second" to unit(0.70f)),
            margin = 0.10f
        )
    ...
    @Test
    fun configuredMarginAppliesEvenWhenSecondCandidateIsBelowThreshold() = runBlocking {
        val diagnostics = mutableListOf<FaceIdentificationPerformance>()
        val engine = engine(
            records = listOf(
