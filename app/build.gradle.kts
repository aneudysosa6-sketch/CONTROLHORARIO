...
val faceEnrollmentUrl = "$supabaseUrl/functions/v1/face-enrollment"

val requestedVariantTasks = gradle.startParameter.taskNames
    .map { it.substringAfterLast(':') }
val debugInvocation = requestedVariantTasks.any { it.contains("Debug", ignoreCase = true) }
val releaseInvocation = requestedVariantTasks.any { it.contains("Release", ignoreCase = true) }
val stagingSupabaseUrl = "https://ertuneccoseqpbkubuet.supabase.co"
val productionSupabaseUrl = "https://heiafcxsvfhinygzxsdk.supabase.co"

if (debugInvocation) {
    require(supabaseUrl == stagingSupabaseUrl) {
        "Las tareas Debug deben apuntar exclusivamente a STAGING"
    }
    require(
        listOf(deviceEnrollmentUrl, employeeSyncUrl, attendanceSyncUrl, faceEnrollmentUrl)
            .none { it.contains("heiafcxsvfhinygzxsdk") }
    ) {
        "Las tareas Debug no pueden contener referencias de PRODUCCION"
    }
}

if (releaseInvocation) {
    require(supabaseUrl == productionSupabaseUrl) {
        "Las tareas Release deben apuntar exclusivamente a PRODUCCION"
    }
    require(
        listOf(deviceEnrollmentUrl, employeeSyncUrl, attendanceSyncUrl, faceEnrollmentUrl)
            .all { it.startsWith("$productionSupabaseUrl/functions/v1/") }
    ) {
        "Todos los endpoints Release deben pertenecer al proyecto Supabase de PRODUCCION"
    }
    require(
        listOf(deviceEnrollmentUrl, employeeSyncUrl, attendanceSyncUrl, faceEnrollmentUrl)
            .none { it.contains("ertuneccoseqpbkubuet") }
    ) {
        "Las tareas Release no pueden contener referencias de STAGING"
    }
    require(supabasePublishableKey != "sb_publishable_dummy") {
        "Release no puede usar una clave publicable dummy"
    }
}
...
require(faceEnrollmentUrl.startsWith("https://") && faceEnrollmentUrl.endsWith("/functions/v1/face-enrollment")) { "face-enrollment debe usar el mismo proyecto Supabase configurado" }
...
android {
    ...
    defaultConfig {
        ...
        resValue("string", "face_enrollment_url", faceEnrollmentUrl)
