...
## ADR-012 - Registro facial exclusivo del Terminal Android
...
El flujo QR dependía de un navegador personal y podía separar el canje del token de la captura real.

Decisión:

El primer registro facial ocurre únicamente en un Terminal Android autorizado. Web conserva estado
y reset administrativo; no crea invitaciones. La confirmación es online, firmada P-256, idempotente,
ligada a empresa/dispositivo/empleado y revalida alcance, horario, día libre y duplicidad.

Consecuencias:

- las invitaciones QR pendientes se revocan y los endpoints históricos fallan cerrados;
- un teléfono personal o JWT normal no puede enrolar;
- Android instala únicamente la plantilla confirmada por PostgreSQL;
- el registro no inicia una jornada y requiere un reconocimiento posterior;
- las migraciones 0055 a 0059 permanecen inmutables como historial.

## ADR-016: Terminal Android cerrado y margin facial opcional

**Estado:** Aceptado, 2026-08-26.

Android arranca por código de registro en instalación nueva y por cámara directa en aperturas posteriores. El enrolamiento facial se ejecuta dentro del Terminal autorizado; el QR facial Web se retira. La aceptación facial exige liveness y threshold. El margin best-vs-second es una protección adicional nullable: solo se evalúa cuando está configurado y existe segundo candidato. La administración continúa exclusivamente en Web.
