...
## Estado validado de STAGING

Las migraciones terminal-only y faciales 0055-0061 están validadas en STAGING `[STAGING_PROJECT_REF]`. La credencial de Terminal, tenant, revocación, lease, firma P-256, idempotencia y anti-replay son obligatorios para eventos de jornada. Las invitaciones QR faciales históricas están revocadas y sus RPC quedan inertes/deprecados. PRODUCCIÓN `[PRODUCTION_PROJECT_REF]` no fue tocada.
