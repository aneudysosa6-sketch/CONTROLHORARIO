...
## Regla terminal-only definitiva

- No agregar Login, dashboard, portal de empleado ni administración a Android.
- No permitir jornadas mediante JWT Web, teléfono personal o QR.
- No reactivar el enrolamiento facial QR retirado.
- Mantener credencial de Terminal, P-256, tenant, revocación, lease, idempotencia y anti-replay.
- Tratar `face_match_margin = null` como filtro opcional no configurado, nunca como bloqueo de identificación si threshold y liveness pasan.
- No cambiar automáticamente `face_match_threshold`.
