...
select has_column('public','user_provisioning_audit','target_user_id_snapshot','audit target snapshot exists');
select has_column('public','profiles','access_deleted_at','access tombstone exists');
select col_not_null('public','user_provisioning_audit','target_user_id_snapshot','audit target snapshot is required');
select ok(
  exists (
...
select col_is_unique('public','empleados','perfil_id','employee profile link is unique');
