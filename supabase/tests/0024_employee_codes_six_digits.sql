...
select (pg_get_functiondef('public.preview_next_employee_code_internal(uuid)'::regprocedure) like '%pg_catalog.random()%') as random_allocator \gset
\if :random_allocator
\echo 1..0 # SKIP Superseded by 0047 random employee-code allocation contracts
rollback;
\quit
\endif
select plan(69);

select has_table('public', 'employee_code_normalization_audit', 'employee_code_normalization_audit exists');
select has_table('public', 'employee_code_registry', 'employee_code_registry exists');
select has_table('public', 'employee_code_sequences', 'employee_code_sequences exists');
select has_table('public', 'face_first_enrollment_code_normalization_audit', 'face_first_enrollment_code_normalization_audit exists');
select has_table('public', 'employee_pin_code_verification_audit', 'employee_pin_code_verification_audit exists');
select has_column('public', 'employee_code_normalization_audit', 'pin_strategy', 'pin strategy audit exists');
