...
function receivedScopePayload(body: JsonRecord, base: JsonRecord) {
  const payload = { ...base };
  if (Object.prototype.hasOwnProperty.call(body, 'branch_ids')) {
    if (!Array.isArray(body.branch_ids)) {
      throw new HttpError(400, 'branch_ids debe ser una lista de UUID');
    }
    payload.branch_ids = [...new Set(
      body.branch_ids.map((value, index) => requiredUuid(value, 'branch_ids[' + index + ']')),
    )].sort();
  }
