...
const MAX_BIOMETRIC_PROOF_TTL_MS = 90_000;

async function validProof(publicKey: string, op: Record<string, unknown>, deviceId: string, occurredAt: string) {
  ...
  const occurred = Date.parse(occurredAt), issuedAt = Date.parse(issued), expiresAt = Date.parse(expires);
  if (
    !uuid.test(id) ||
    !uuid.test(employee) ||
    !['INICIAR', 'PAUSAR', 'REANUDAR', 'FINALIZAR'].includes(action) ||
    ![occurred, issuedAt, expiresAt].every(Number.isFinite) ||
    issuedAt > occurred ||
    occurred > expiresAt ||
    expiresAt - issuedAt > MAX_BIOMETRIC_PROOF_TTL_MS
  ) return false;
  try {
    ...
    return crypto.subtle.verify(
      ...
      key,
      derToRaw(b64(signature)),
      new TextEncoder().encode(`${id}|${employee}|${deviceId}|${action}|${issued}|${expires}`),
    );
  } catch {
    return false;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const terminalEligible = async (employeeId: string) => {
      const { data, error } = await admin.rpc('terminal_empleado_elegible', {
        p_empresa: auth.empresa_id,
    ...
    const body = await req.json() as {
      mode?: unknown;
      employee_remote_id?: unknown;
      requested_at?: unknown;
      message_id?: unknown;
      idempotency_key?: unknown;
      operations?: unknown[];
    };
    if (text(body.mode) === 'message_receipt') {
      ...
      if (!uuid.test(employeeRemoteId) || !uuid.test(messageId) || !uuid.test(idempotencyKey)) {
        return json({ error_code: 'INVALID_PAYLOAD', request_id: requestId }, 400);
      }
      const { data: receipt, error: receiptError } = await admin.rpc(
        'confirmar_mensaje_recibido_dispositivo',
        { payload: {
          empresa_id: auth.empresa_id,
          dispositivo_id: deviceId,
          empleado_id: employeeRemoteId,
          mensaje_id: messageId,
          idempotency_key: idempotencyKey,
        } },
      );
      if (receiptError) {
        return json({ error_code: receiptError.code || 'MESSAGE_RECEIPT_ERROR', request_id: requestId }, 409);
      }
      const receiptRecord = (receipt ?? {}) as Record<string, unknown>;
      const audioObjectPath = text(receiptRecord.audio_object_path);
      if (audioObjectPath) await admin.storage.from('employee-message-audio').remove([audioObjectPath]);
      return json({
        result: receiptRecord.result,
        message_id: receiptRecord.message_id,
    ...
    if (text(body.mode) === 'current_state') {
      ...
      if (!await terminalEligible(employeeRemoteId)) {
        return json({ error_code: 'TERMINAL_DEPARTMENT_UNAUTHORIZED', message: 'TERMINAL NO AUTORIZADO PARA SU DEPARTAMENTO', request_id: requestId }, 403);
      }
    ...
    for (const raw of body.operations) {
      ...
      if (!uuid.test(key) || !uuid.test(employee) || !['INICIAR', 'PAUSAR', 'REANUDAR', 'FINALIZAR'].includes(action) || !/^\d{4}-\d{2}-\d{2}$/.test(workDate) || !validIsoInstant(occurred)) {
        results.push({ idempotency_key: key, result: 'rejected', error_code: 'INVALID_PAYLOAD' });
        continue;
      }
      if (!await terminalEligible(employee)) {
        results.push({ idempotency_key: key, result: 'rejected', error_code: 'TERMINAL_DEPARTMENT_UNAUTHORIZED', message: 'TERMINAL NO AUTORIZADO PARA SU DEPARTAMENTO' });
        continue;
      }
      if (!await validProof(device.public_key_spki, op, deviceId, occurred)) {
      ...
      let employeeMessage: Record<string, unknown> | null = null;
      if (['accepted', 'duplicate'].includes(text(operationResult.result))) {
        const { data: pendingMessage, error: messageError } = await admin.rpc(
          'obtener_mensaje_pendiente_dispositivo',
          { p_empresa: auth.empresa_id, p_dispositivo: deviceId, p_empleado: employee },
        );
        if (!messageError && pendingMessage) {
          employeeMessage = pendingMessage as Record<string, unknown>;
          const audioObjectPath = text(employeeMessage.audio_object_path);
          if (audioObjectPath) {
            const { data: signed } = await admin.storage.from('employee-message-audio')
              .createSignedUrl(audioObjectPath, 600);
            employeeMessage = {
              ...employeeMessage,
              audio_url: signed?.signedUrl ?? null,
              audio_object_path: undefined,
            };
          }
        }
      }
      debug('PUNCH_API', {
        ...
        employeeId: employee,
        action,
        result: operationResult.result ?? 'accepted',
        messagePending: employeeMessage !== null,
      });
      results.push({
        idempotency_key: key,
        ...operationResult,
        ...(employeeMessage ? { message: employeeMessage } : {}),
      });
    }
    await admin.from('credenciales_dispositivo').update({ ultima_uso_at: now }).eq('dispositivo_id', deviceId);
    await admin.from('dispositivos_android').update({ ultima_conexion_at: now }).eq('id', deviceId);
